...
- **自製字幕檔匯入〔#193，v3.6.0〕**：【獲得】頁除了貼網址，**多一條完全不必上網的路**——把你自己做的字幕檔直接給它。
  ...
  4. 按「**⚙ 加入影片**」。**單檔**＝看一眼摘要（幾句、哪些說話人、多長）確認後直接載入播放；**多檔**＝先出**彙總確認表**逐檔列清楚，確認後批次加入【內容】清單（**不會自動播放**，你自己挑要看哪支）。
  ...
  **讓字幕檔自己帶影片網址**（推薦，一個檔就是一課的完整資料）：在檔頭加一行註解寫上該片網址即可，程式會自動認出來——
  ...
  >
  > **不花錢**：自製 SRT 自帶時間軸，走**免費解析**、完全不用 AI。批次匯入時就算某個檔沒有時間軸，也只會標「需 AI 抽取·已略過」**不會自動幫你花錢**；已經加過的影片預設略過、不重建。

  ![影片頁【獲得】子頁（v3.6.0）：單一輸入框可拖放字幕檔＋「選擇字幕檔…」鈕＋已選檔列（顯示自檔內偵測到的影片 ID）＋多檔批次加入說明](docs/design-visual/page-影片擷取頁.svg)
  > 設計期參考稿·待 code 階段以實際截圖替換。
