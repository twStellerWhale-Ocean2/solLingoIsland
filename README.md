...
| 我的筆記 | 主視窗「筆記」分頁：左側**多層資料夾樹如檔案總管**——頂部「建立資料夾」、節點**右鍵選單**（新增子資料夾／更名〔`F2` 原地編輯〕／刪除〔`Del`〕）、**同層自動依名稱排序**、拖曳只改所屬資料夾（目標夾高亮）；右側該夾條目可拖曳排序（顯示插入位置線）**或拖到左側資料夾改歸屬**，**每卡原文下小字顯示登記時間**（很舊的版本收藏、沒有時間紀錄的筆記不顯示）；右上排序鈕 **A→Z／Z→A**（依原文）與 **Old→New／New→Old**（依登記時間，皆即存、重啟沿用），加上「**Clear Practice（清空練習紀錄）**」把該夾所有成績框歸零；條目**右鍵**＝`▶ 播音`／`檢視`／`底色`／`刪除`，**雙擊＝檢視**，**行尾＝播音鈕｜麥克風鈕｜成績框** |
