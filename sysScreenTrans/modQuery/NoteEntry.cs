...
/// <summary>
/// 單筆我的筆記（[modQuery模組] 我的筆記儲存契約，spec#7）：唯一 Id＋加入時間＋三欄結果。
/// 去重以 <see cref="Key"/>（英文原文正規化：去頭尾空白、大小寫折疊）為準；金鑰不入筆記。
/// </summary>
public sealed record NoteEntry(string Id, DateTimeOffset AddedAt, string Original, string Phonetic, string Translation)
{
    /// <summary>還原為呈現層三欄結果（供筆記「檢視」開結果卡片詳情）。</summary>
    public QueryResult ToResult() => new(Original, Phonetic, Translation);

    /// <summary>由一筆查詢結果與當下時間建立筆記（Id 隨機、供定位）。</summary>
