...
| 統一主視窗（Office 式殼） | 工具維保／teamSop#2.2 | 維運 | **標準工作列視窗**（`ShowInTaskbar`、可最小化收合、關閉（✕）＝收合非結束、點工作列／Alt+Tab 還原）：**頂部功能列分頁**（每個分頁鈕採**圖示在上、文字在下**〔📒筆記／🕘歷史／⚙選項／ℹ關於；實作以 Segoe MDL2 Assets 字形圖示或 emoji〕）＋下方對應功能頁；淺粉底（承結果卡片色）＋小女孩 logo 背景浮水印；含金鑰狀態列。取代原各獨立維運/檢視視窗，為第一線常顯主控入口 | #2.2.1 | 桌面視窗（工作列按鈕、分頁殼） |
