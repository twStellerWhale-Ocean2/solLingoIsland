...
* **條目顯示偏好**（設定頁「條目顯示」，存 appsettings）：`paramEntryFontSize`（筆記/歷史條目原文字級，預設 18、界 8–48）、`paramEntryBold`（粗體，預設 true）、`paramEntryWrap`（自動換行，預設 false＝單行省略）。
