...
* **結果視窗**：**標準表單**（OS 標準標題列＋標準邊框拖拉縮放＋工作列按鈕，字型比照主視窗；Issue #59）、淺粉底大字；直排原文／KK 音標／中譯（**不加欄目標示**——三區以字級/色彩/字體分層、一望即知），英文組與中文組各有獨立播放鈕與「自動播放」勾選（勾選後框選完即朗讀）；英文原文可**逐字點選**——點某個單字即單獨發音（游標呈可點狀、整句播放鈕並存）；首次置中、之後記住使用者擺放的位置與大小；以標準標題列拖曳移動、標準邊框拖拉縮放；topmost 浮於遊戲上「一直存在」，**失焦（切換至其他視窗對照）不自動關閉、不隱藏**（有工作列按鈕可尋），**點選主視窗／經 tray 開主視窗分頁亦不關閉**（與主視窗共存，Issue #105）；關閉時機僅限 `ESC`／標題列關閉鈕／下一次查詢取代／選項儲存後重建；同時至多一個。
...
| 40 | runWi自訂Usr查看聆聽結果（主視窗共存） | 07 | 開啟結果視窗後**點選主視窗**／自工作列還原主視窗／經 tray 開任一主視窗分頁 → 結果視窗**保留不自動關閉**（Issue #105，移除主視窗 `Activated` 與 `OpenMain` 之關閉路徑）；關閉時機僅限：使用者關閉（`ESC`／關閉鈕）、新查詢取代（同時至多一個）、選項儲存後重建；上述主視窗操作後結果視窗播音、加入筆記如常 |
