...
| 40 | runWi自訂Usr查看聆聽結果（主視窗共存） | 07 | 開啟結果視窗後**點選主視窗**／自工作列還原主視窗／經 tray 開任一主視窗分頁 → 結果視窗**保留不自動關閉**（Issue #105，移除主視窗 `Activated` 與 `OpenMain` 之關閉路徑）；結果卡開啟中自歷史/筆記**「檢視」**開詳情卡 → **前卡被取代、仍僅一張**（單一守衛、無孤兒卡）；關閉時機僅限：使用者關閉（`ESC`／關閉鈕）、新查詢或「檢視」取代（同時至多一個）、選項儲存後（語音服務重建而）關閉不重開；上述主視窗操作後結果視窗播音、加入筆記如常 |
