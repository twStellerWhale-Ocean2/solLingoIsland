...
* **本增量（v2.0.0 影片來源升格）**：本方案由「螢幕翻譯工具」升格為**多來源英文擷取練習中樞**——螢幕與 **YouTube 影片**為可插拔擷取來源，下游查詢／筆記／發音練習共用。本增量新增**影片「學習查詢」主動線**（Mode A，`spec#11`–`spec#15`）：選定 YouTube 影片、自動取得字幕、導引播放到句暫停顯字幕、於暫停句點字查詢並加入既有「我的筆記」；**不含**說話者標註、代言練習、完整影片庫（後續增量另立工單）。
...
SOL ==>|"辨識翻譯查詢"| OPENAI
SOL ==>|"原文朗讀"| SPEECH
SOL ==>|"取字幕／導引播放(yt-dlp／WebView2，本增量)"| YT
```
...
| YouTube影片 | 查詢對象（本增量） | 使用者自選之英文影片來源；本工具僅取其字幕文字、不下載影片內容（來源使用者自負，比照自備金鑰） |
...
| --- | --- | --- |
| **orgsopcat#1-英文擷取·回顧·收藏·練習** | orgSop#1-畫面選區查詢 | 遊戲中熱鍵喚起、框選畫面區塊、取得原文／音標／中譯、查看聆聽後返回遊戲 |
...
| | orgSop#7-影片擷取學習查詢 | 選定 YouTube 影片為擷取來源、自動取得字幕、導引播放到句暫停顯字幕、於暫停句點字查詢、將擷取之字／句加入我的筆記（沿用去重／資料夾／發音練習）；螢幕與影片為並列之可插拔擷取來源 |
...
* **spec#11-可選定 YouTube 影片為擷取來源**：使用者於主視窗「影片擷取」分頁貼入或指定一支 YouTube 影片，作為與螢幕並列之新英文擷取來源；全程於本工具內完成，不需另開瀏覽器操作。
* **spec#12-可自動取得該影片逐句字幕**：選定影片後系統自動取得其英文字幕（逐句文字＋起訖時間）；以字幕文字為準、免螢幕辨識（OCR）；字幕不可用（無字幕／取得失敗）時明確告知並中止該片、不當機不無聲失敗。
* **spec#13-可導引播放並到句暫停顯示字幕**：以導引方式播放該影片，播放至每句（有人說話）時自動暫停、於畫面下方顯示該句英文字幕；使用者可續播、重播該句、跳至下一句，暫停期不誤放過該句。
* **spec#14-可於暫停句點字即時查詢**：於暫停顯示之字幕句中點選任一英文單字，即以既有查詢取得該字之原文／KK 音標／繁體中文翻譯並可聆聽（沿用 `spec#3`／`spec#4` 之查詢與朗讀，來源改為影片字幕文字、免辨識誤差）。
* **spec#15-可將影片擷取內容加入我的筆記**：可將影片擷取到的單字或整句加入既有「我的筆記」，沿用既有去重、資料夾分類、拖曳排序與逐則發音練習（`spec#7`／`spec#10`）；下游筆記與練習完全共用、不另造一套。
...
* **e2eTest#07-影片擷取學習查詢一圈**（依 orgSop#7、docProgTest#07）：於「影片擷取」分頁貼入一支有英文字幕之 YouTube 影片 → 自動取得字幕 → 導引播放至某句自動暫停、畫面下方顯示該句字幕 → 點該句某英文單字查得原文／KK／中譯並聆聽 → 將該字（或整句）加入我的筆記、於筆記分頁見該筆並可發音練習；另試一支無字幕影片 → 明確告知並中止 → 全圈閉合、查詢與筆記後段沿用既有、字幕不可用明確降級不當機。
...
* **spec#11**：貼入 YouTube 連結／ID 之解析成功率、非法或私人影片之明確錯誤提示、影片載入至可播之延遲。
* **spec#12**：字幕自動取得成功率（有字幕影片）、無字幕／取得失敗之明確告知與中止正確性（不當機、不空白卡住）、字幕逐句文字與時間解析正確率。
* **spec#13**：到句暫停之命中率與時間誤差（目標 ±0.2s 內、不漏句不早停）、續播／重播該句／跳下一句操作正確、暫停時字幕與當前句一致。
* **spec#14**：暫停句點字之命中正確性（點中的詞即為查詢詞、標點不誤取）、查得三欄齊備率、沿用既有查詢之延遲與朗讀成功率。
* **spec#15**：影片擷取之字／句加入筆記之去重與歸類正確、於筆記分頁可見並可發音練習（沿用 spec#7／spec#10 行為）、與螢幕來源之筆記無差別共存。
...
| team | 上級／編成位置 | 職責 |
| --- | --- | --- |
| team擷取學習（#1／#3／#4／#5／#6／#7） | Org使用者（遊戲中/回顧/整理時） | 熱鍵喚起、框選查詢、查看聆聽結果；回顧查詢歷史；收藏並整理我的筆記；建立管理應用情境並擇一使用；就地錄音練習發音 |
...
| --- | --- |
| team擷取學習（#1／#3／#4／#5／#6／#7） | teamSop#1.1-熱鍵喚起與框選擷取<br>teamSop#1.2-辨識翻譯查詢<br>teamSop#1.3-結果查看與朗讀<br>teamSop#3.1-開啟並瀏覽查詢歷史<br>teamSop#3.2-查詢歷史清理<br>teamSop#4.1-收藏加入我的筆記<br>teamSop#4.2-筆記整理（資料夾/排序/檢視/刪除）<br>teamSop#5.1-情境建立與編輯（含圖片自動解釋）<br>teamSop#5.2-情境擇一使用<br>teamSop#6.1-筆記發音練習（錄音→AI 評分→成績框回饋→清空練習紀錄）<br>teamSop#7.1-影片選定與字幕預處理<br>teamSop#7.2-導引播放到句暫停查詢<br>teamSop#7.3-影片擷取加入筆記 |
...
**導覽衍生（IA ⟵ SOP；硬規則④之桌面對應）**：`teamSop#1.1→選區遮罩頁`、`teamSop#1.2／#1.3→查詢結果頁`、`teamSop#2.2／#2.3→統一主視窗（選項／關於分頁；系統匣選單頁為輔助鏡像）`、`teamSop#3.1／#3.2→主視窗歷史分頁`、`teamSop#4.1→（結果頁底部「加入我的筆記」/「自動加入筆記」＋歷史分頁「＋筆記」）`、`teamSop#4.2→主視窗筆記分頁`、`teamSop#6.1→主視窗筆記分頁（卡片內就地錄音練習）`、`teamSop#5.1／#5.2→主視窗情境分頁`；`teamSop#7.1／#7.2／#7.3→主視窗影片擷取分頁`（與螢幕擷取〔情境〕頁並列之擷取來源頁）；`teamSop#2.1`（安裝金鑰）走 OS 標準設定、不在本系統 UI 內。
...
* **docProgTest#07-影片擷取學習查詢**（orgSop#7）：開影片擷取分頁、貼入 YouTube 影片、取得字幕、導引播放到句暫停顯字幕、點字查詢、加入我的筆記；無字幕影片明確告知並中止。
...
| --- | --- | --- |
| **team擷取學習（#1／#3／#4／#5／#6／#7）**<br>prsn玩家 | teamSop#1.1 | prsnSop#1.1.1〔prsn玩家·選區遮罩頁〕熱鍵喚起並框選〔[runWi自訂Usr熱鍵喚起框選]〕 |
...
| | teamSop#7.1 | prsnSop#7.1.1〔prsn玩家·影片擷取頁〕貼入 YouTube 影片、自動取得字幕、預處理為逐句〔[runWi自訂Usr選片取字幕]〕 |
| | teamSop#7.2 | prsnSop#7.2.1〔prsn玩家·影片擷取頁〕導引播放到句暫停顯字幕、於暫停句點字查詢〔[runWi自訂Usr導引播放點字查詢]〕 |
| | teamSop#7.3 | prsnSop#7.3.1〔prsn玩家·影片擷取頁〕將擷取之字／句加入我的筆記〔[runWi自訂Usr影片擷取加筆記]〕 |
...
| 51 | runWi自訂Usr導引播放點字查詢（點字查詢） | 50 | 暫停句字幕逐字可點 → 點某英文單字以既有 `QueryService` 查得三欄並可朗讀（沿用 spec#3／#4、來源改字幕文字）；含標點之詞剝標點取原詞（沿用 `EnglishWordTokenizer`）；查詢降級／重試沿用既有（測試攔截、不打真網路）|
