...
| 42 | runWi自訂Usr管理我的筆記（登記時間標示/時間排序） | 30 | 筆記卡片原文下顯示**登記時間**小字（`AddedAt` 本地時間 `yyyy-MM-dd HH:mm`；無值不顯示、不佔位）；右欄頂排序鈕四顆 `A→Z`/`Z→A`/`Old→New`/`New→Old`（原[順向排序]/[反向排序]標籤收斂，行為不變）＋[清空練習紀錄]，空夾五鈕停用；按 `Old→New` → 目前夾條目依登記時間**舊→新**排序並即時落地 `notes.json`、重啟沿用；按 `New→Old` → 新→舊；同時刻多筆維持相對順序（**穩定排序**、`SortEntriesByTime` 純函式測涵蓋）；排序後仍可拖曳微調（與 #52 一致） |
