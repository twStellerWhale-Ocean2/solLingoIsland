...
* **sys 下屬 module**：[modCapture模組]、[modQuery模組]、[modPresent模組]（皆隸屬單一 WPF exe；[techStackDotnetWin] 候選）。
  ...
  * **[modPresent模組] 呈現契約**（spec#4）：結果視窗 topmost；首次置中、之後記住使用者擺放的位置與大小（跨啟動、存 `%APPDATA%\ScreenTrans\ui-state.json`）；可拖曳標題移動、右下握把縮放；TTS（Windows 內建語音 SAPI，語音可於設定選擇）非同步播放、中英可各自播放與自動播放、重複觸發先停再播；朗讀之 SSML prompt **句首墊一小段靜音**（實作內常數，非使用者組態）以吸收 SAPI 起始 warm-up 削音；`ESC`／點外即關。**invariant**：UI 執行緒不阻塞；關閉後無殘影視窗；**朗讀自首音節起完整**（不因引擎句首 warm-up 削掉起始音節）。
