...
| 41 | runWi自訂Usr查看聆聽結果（主視窗喚回） | 13 | 主視窗功能列右端 `Result` 鈕（Issue #107）三態：結果卡**開啟中**按下→該卡帶至前景、不新開（仍僅一張）；卡**已關閉**且查詢歷史有紀錄→以最新一筆重開三欄詳情（走「檢視」單一取代守衛）；**無任何歷史**→右下 toast `No query result yet`、不開卡；鈕為動作鈕非分頁、按下不改變當前分頁選取 |
