...
* **本增量（v1.1.0 影片來源升格）**：本方案由「螢幕翻譯工具」升格為**多來源英文擷取練習中樞**——螢幕與 **YouTube 影片**為可插拔擷取來源，下游查詢／筆記／發音練習共用。本增量新增**影片「學習查詢」主動線**（Mode A，`spec#2`–`spec#2`）：選定 YouTube 影片、自動取得字幕、導引播放到句暫停顯字幕、於暫停句點字查詢並加入既有「我的筆記」；**不含**說話者標註、代言練習、完整影片庫（後續增量另立工單）。
