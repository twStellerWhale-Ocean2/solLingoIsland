...
public partial class VideoCapturePage : System.Windows.Controls.UserControl
{
    ...
    private async Task LoadAsync()
    {
        ...
        try
        {
            var result = await _fetcher.FetchAsync(UrlBox.Text);
            _cues = result.Cues;
            ...
            if (_webReady)
            {
                ...
                _poll.Start();
                SetControls(true);
                SetStatus(result.IsAutoGenerated
                    ? $"{_cues.Count} auto-generated caption lines — machine-transcribed and often fragmented, so a video with proper subtitles works best. Playback still pauses at each line."
